...
def main(beta, num_clients) -> None:
    ...
    model = cifar.Net()
    model.to(DEVICE)
    _, testloader, num_examples = cifar.load_data()
    import pickle
    if beta == 'iid':
        with open(f'./cifar_client/iid/num_client={num_clients}/X_train.pkl', 'rb') as f:
            X = pickle.load(f)[worker_id]
        with open(f'./cifar_client/iid/num_client={num_clients}/y_train.pkl', 'rb') as f:
            y = pickle.load(f)[worker_id]
    else:
        with open(f'./cifar_client/alpha={beta}/X_train.pkl', 'rb') as f:
            X = pickle.load(f)[worker_id]
        with open(f'./cifar_client/alpha={beta}/y_train.pkl', 'rb') as f:
            y = pickle.load(f)[worker_id]
...
if __name__ == "__main__":
    ...
    # time.sleep(10)
    # print(f'Memory Usage ov {worker_id}:', memory_usage(process, device=CVD))
    ...
    with open(f'{output_cifar}/Results_{server_IP}_{num_clients}/Client{worker_id}_{beta}_performance.txt', 'w') as f:
        ...
        f.write(f'Execution time mean: {tempo_medio} seconds\n')
