...
def create_data_client(num_clients, num_classes, least_samples=1024, niid=False, partition='pat',
    ...
    trainloader, testloader, num_examples = fashion_mnist.load_data()
    x_train_list = []
    y_train_list = []
...
if __name__ == '__main__':
    ...
    if niid:
        balance = False
        fig_path = f'./fmnist_client/alpha={alpha}/'
    else:
        balance = True
        fig_path = f'./fmnist_client/iid/num_client={num_clients}/'
    os.makedirs(fig_path, exist_ok=True)

    X, y, statistics = create_data_client(num_clients, num_classes, least_samples=least_samples,
                                          niid=niid, partition=partition,
