...
_, _, total_examples = fashion_mnist.load_data()
...
list_files = os.listdir(f'Results_{server_IP}_{num_clients}')
# open file start with client
list_files = [file for file in list_files if file.startswith('Client')]
efficiency_client = []
memory_client = []
for f in list_files:
    with open(f'Results_{server_IP}_{num_clients}/{f}', 'r') as file:
...
os.makedirs(f'Results_{server_IP}_{num_clients}', exist_ok=True)
with open(f'Results_{server_IP}_{num_clients}/Server_performance.txt', 'w') as f:
